...
## ============================================
## New Environment Sound Integration Tests (Round 3)
## ============================================
func _test_new_env_sounds_round3() -> void:
	print("\n--- New Environment Sound Integration Tests (Round 3) ---")

	# Test 1: New environment sound files exist
	var new_sounds = ["env_aurora_snowfield", "env_cherry_blossom_valley", "env_moonlit_garden", "env_mountaintop", "env_mushroom_forest", "env_night", "env_ocean", "env_pond", "env_sakura_shrine", "env_savanna"]
	for sound_name in new_sounds:
		var path = "res://assets/audio/environment/%s.wav" % sound_name
		_assert(FileAccess.file_exists(path), "Sound file exists: %s" % sound_name)
	...
	# Test 3: AudioManager has env_night sound
	_assert(AudioManager._sound_paths.has("env_night"), "AudioManager has env_night")

	# Test 4: AudioManager has env_ocean sound
	_assert(AudioManager._sound_paths.has("env_ocean"), "AudioManager has env_ocean")

	# Test 5: AudioManager has env_mushroom_forest sound
	_assert(AudioManager._sound_paths.has("env_mushroom_forest"), "AudioManager has env_mushroom_forest")
	...
	for sound_name in AudioManager._sound_paths.keys():
		if sound_name.begins_with("env_"):
			env_count += 1
	_assert(env_count >= 55, "Environment sound count >= 55 (actual: %d)" % env_count)

	# Test 7: Total sound count increased
	_assert(AudioManager._sound_paths.size() >= 250, "Total sound count >= 250 (actual: %d)" % AudioManager._sound_paths.size())

	# Test 8: New sounds have correct file paths
	_assert(AudioManager._sound_paths["env_night"] == "res://assets/audio/environment/env_night.wav", "env_night path correct")
	_assert(AudioManager._sound_paths["env_ocean"] == "res://assets/audio/environment/env_ocean.wav", "env_ocean path correct")
	_assert(AudioManager._sound_paths["env_mushroom_forest"] == "res://assets/audio/environment/env_mushroom_forest.wav", "env_mushroom_forest path correct")

	# Test 9: AudioManager can play new sounds (no crash)
	AudioManager.play_sfx("env_night")
	AudioManager.play_sfx("env_ocean")
	AudioManager.play_sfx("env_mushroom_forest")
	_assert(true, "New environment sounds play without crash")
	...
	# Test 11: New environment sound names are descriptive
	_assert(new_sounds[0] == "env_aurora_snowfield", "First new sound is env_aurora_snowfield")
	_assert(new_sounds[5] == "env_night", "Sixth new sound is env_night")
	_assert(new_sounds[9] == "env_savanna", "Tenth new sound is env_savanna")

	# Test 12: New environment sounds cover diverse biomes
	var biomes = ["snow", "cherry", "moon", "mountain", "mushroom", "night", "ocean", "pond", "sakura", "savanna"]
	for biome in biomes:
		var found = false
		for sound_name in new_sounds:
			if sound_name.find(biome) >= 0:
...
## ============================================
## New Soul Sound Integration Tests (Round 3)
## ============================================
func _test_new_soul_sounds_round3() -> void:
	print("\n--- New Soul Sound Integration Tests (Round 3) ---")

	# Test 1: New soul sound files exist
	var new_sounds = ["soul_ashamed", "soul_enthusiastic", "soul_evolve", "soul_excited_bounce", "soul_excited_sparkle", "soul_expectant", "soul_fear_tremble", "soul_fierce", "soul_focused_concentrate", "soul_forgiving"]
	for sound_name in new_sounds:
		var path = "res://assets/audio/soul/%s.wav" % sound_name
		_assert(FileAccess.file_exists(path), "Sound file exists: %s" % sound_name)
	...
	# Test 3: AudioManager has soul_evolve sound
	_assert(AudioManager._sound_paths.has("soul_evolve"), "AudioManager has soul_evolve")

	# Test 4: AudioManager has soul_fierce sound
	_assert(AudioManager._sound_paths.has("soul_fierce"), "AudioManager has soul_fierce")

	# Test 5: AudioManager has soul_fear_tremble sound
	_assert(AudioManager._sound_paths.has("soul_fear_tremble"), "AudioManager has soul_fear_tremble")
	...
	for sound_name in AudioManager._sound_paths.keys():
		if sound_name.begins_with("soul_"):
			soul_count += 1
	_assert(soul_count >= 65, "Soul sound count >= 65 (actual: %d)" % soul_count)

	# Test 7: Total sound count increased
	_assert(AudioManager._sound_paths.size() >= 260, "Total sound count >= 260 (actual: %d)" % AudioManager._sound_paths.size())

	# Test 8: New sounds have correct file paths
	_assert(AudioManager._sound_paths["soul_evolve"] == "res://assets/audio/soul/soul_evolve.wav", "soul_evolve path correct")
	_assert(AudioManager._sound_paths["soul_fierce"] == "res://assets/audio/soul/soul_fierce.wav", "soul_fierce path correct")
	_assert(AudioManager._sound_paths["soul_fear_tremble"] == "res://assets/audio/soul/soul_fear_tremble.wav", "soul_fear_tremble path correct")

	# Test 9: AudioManager can play new sounds (no crash)
	AudioManager.play_sfx("soul_evolve")
	AudioManager.play_sfx("soul_fierce")
	AudioManager.play_sfx("soul_fear_tremble")
	_assert(true, "New soul sounds play without crash")
	...
	_assert(new_sounds[0] == "soul_ashamed", "First new sound is soul_ashamed")
	_assert(new_sounds[5] == "soul_expectant", "Sixth new sound is soul_expectant")
	_assert(new_sounds[9] == "soul_forgiving", "Tenth new sound is soul_forgiving")

	# Test 12: New soul sounds cover diverse emotions
	var emotions = ["ashamed", "enthusiastic", "evolve", "excited", "expectant", "fear", "fierce", "focused", "forgiving"]
...
## ============================================
## New Soul Sound Integration Tests (Round 4)
## ============================================
func _test_new_soul_sounds_round4() -> void:
	print("\n--- New Soul Sound Integration Tests (Round 4) ---")

	# Test 1: New soul sound files exist
	var new_sounds = ["soul_confused_tilt", "soul_disheartened", "soul_furious", "soul_gallant", "soul_generous", "soul_gracious", "soul_grateful_thanks", "soul_grow_up", "soul_guilty", "soul_happy_chime"]
	for sound_name in new_sounds:
		var path = "res://assets/audio/soul/%s.wav" % sound_name
		_assert(FileAccess.file_exists(path), "Sound file exists: %s" % sound_name)
	...
	# Test 3: AudioManager has soul_furious sound
	_assert(AudioManager._sound_paths.has("soul_furious"), "AudioManager has soul_furious")

	# Test 4: AudioManager has soul_gallant sound
	_assert(AudioManager._sound_paths.has("soul_gallant"), "AudioManager has soul_gallant")

	# Test 5: AudioManager has soul_happy_chime sound
	_assert(AudioManager._sound_paths.has("soul_happy_chime"), "AudioManager has soul_happy_chime")
	...
	for sound_name in AudioManager._sound_paths.keys():
		if sound_name.begins_with("soul_"):
			soul_count += 1
	_assert(soul_count >= 75, "Soul sound count >= 75 (actual: %d)" % soul_count)

	# Test 7: Total sound count increased
	_assert(AudioManager._sound_paths.size() >= 288, "Total sound count >= 288 (actual: %d)" % AudioManager._sound_paths.size())

	# Test 8: New sounds have correct file paths
	_assert(AudioManager._sound_paths["soul_furious"] == "res://assets/audio/soul/soul_furious.wav", "soul_furious path correct")
	_assert(AudioManager._sound_paths["soul_gallant"] == "res://assets/audio/soul/soul_gallant.wav", "soul_gallant path correct")
	_assert(AudioManager._sound_paths["soul_happy_chime"] == "res://assets/audio/soul/soul_happy_chime.wav", "soul_happy_chime path correct")

	# Test 9: AudioManager can play new sounds (no crash)
	AudioManager.play_sfx("soul_furious")
	AudioManager.play_sfx("soul_gallant")
	AudioManager.play_sfx("soul_happy_chime")
	_assert(true, "New soul sounds play without crash")
	...
	# Test 11: New soul sound names are descriptive
	_assert(new_sounds[0] == "soul_confused_tilt", "First new sound is soul_confused_tilt")
	_assert(new_sounds[5] == "soul_gracious", "Sixth new sound is soul_gracious")
	_assert(new_sounds[9] == "soul_happy_chime", "Tenth new sound is soul_happy_chime")

	# Test 12: New soul sounds cover diverse emotions
	var emotions = ["confused", "disheartened", "furious", "gallant", "generous", "gracious", "grateful", "grow", "guilty", "happy"]
	for emotion in emotions:
		var found = false
		for sound_name in new_sounds:
			if sound_name.find(emotion) >= 0:
...
## ============================================
## New Environment Sound Integration Tests (Round 5)
## ============================================
func _test_new_env_sounds_round5() -> void:
	print("\n--- New Environment Sound Integration Tests (Round 5) ---")

	# Test 1: New environment sound files exist
	var new_sounds = ["env_underground_city", "env_underground_river", "env_volcanic_wasteland", "env_volcano", "env_waterfall_canyon", "env_wetland", "env_wind_erosion"]
	for sound_name in new_sounds:
		var path = "res://assets/audio/environment/%s.wav" % sound_name
		_assert(FileAccess.file_exists(path), "Sound file exists: %s" % sound_name)
	...
	# Test 3: AudioManager has env_volcano sound
	_assert(AudioManager._sound_paths.has("env_volcano"), "AudioManager has env_volcano")

	# Test 4: AudioManager has env_waterfall_canyon sound
	_assert(AudioManager._sound_paths.has("env_waterfall_canyon"), "AudioManager has env_waterfall_canyon")

	# Test 5: AudioManager has env_wetland sound
	_assert(AudioManager._sound_paths.has("env_wetland"), "AudioManager has env_wetland")
	...
	for sound_name in AudioManager._sound_paths.keys():
		if sound_name.begins_with("env_"):
			env_count += 1
	_assert(env_count >= 72, "Environment sound count >= 72 (actual: %d)" % env_count)

	# Test 7: Total sound count increased
	_assert(AudioManager._sound_paths.size() >= 295, "Total sound count >= 295 (actual: %d)" % AudioManager._sound_paths.size())

	# Test 8: New sounds have correct file paths
	_assert(AudioManager._sound_paths["env_volcano"] == "res://assets/audio/environment/env_volcano.wav", "env_volcano path correct")
	_assert(AudioManager._sound_paths["env_waterfall_canyon"] == "res://assets/audio/environment/env_waterfall_canyon.wav", "env_waterfall_canyon path correct")
	_assert(AudioManager._sound_paths["env_wetland"] == "res://assets/audio/environment/env_wetland.wav", "env_wetland path correct")

	# Test 9: AudioManager can play new sounds (no crash)
	AudioManager.play_sfx("env_volcano")
	AudioManager.play_sfx("env_waterfall_canyon")
	AudioManager.play_sfx("env_wetland")
	_assert(true, "New environment sounds play without crash")
	...
	# Test 11: New environment sound names are descriptive
	_assert(new_sounds[0] == "env_underground_city", "First new sound is env_underground_city")
	_assert(new_sounds[3] == "env_volcano", "Fourth new sound is env_volcano")
	_assert(new_sounds[6] == "env_wind_erosion", "Seventh new sound is env_wind_erosion")

	# Test 12: New environment sounds cover diverse biomes
	var biomes = ["underground", "volcanic", "volcano", "waterfall", "wetland", "wind"]
	for biome in biomes:
		var found = false
		for sound_name in new_sounds:
			if sound_name.find(biome) >= 0:
...
## ============================================
## New Soul Sound Integration Tests (Round 6)
## ============================================
func _test_new_soul_sounds_round6() -> void:
	print("\n--- New Soul Sound Integration Tests (Round 6) ---")

	# Test 1: New soul sound files exist
	var new_sounds = ["soul_lonely_whisper", "soul_loyal", "soul_magnanimous", "soul_merciful", "soul_motivated", "soul_nostalgic_memory", "soul_optimistic", "soul_passionate", "soul_peaceful_calm", "soul_pet"]
	for sound_name in new_sounds:
		var path = "res://assets/audio/soul/%s.wav" % sound_name
		_assert(FileAccess.file_exists(path), "Sound file exists: %s" % sound_name)
	...
	# Test 3: AudioManager has soul_loyal sound
	_assert(AudioManager._sound_paths.has("soul_loyal"), "AudioManager has soul_loyal")

	# Test 4: AudioManager has soul_optimistic sound
	_assert(AudioManager._sound_paths.has("soul_optimistic"), "AudioManager has soul_optimistic")

	# Test 5: AudioManager has soul_passionate sound
	_assert(AudioManager._sound_paths.has("soul_passionate"), "AudioManager has soul_passionate")
	...
	for sound_name in AudioManager._sound_paths.keys():
		if sound_name.begins_with("soul_"):
			soul_count += 1
	_assert(soul_count >= 95, "Soul sound count >= 95 (actual: %d)" % soul_count)

	# Test 7: Total sound count increased
	_assert(AudioManager._sound_paths.size() >= 315, "Total sound count >= 315 (actual: %d)" % AudioManager._sound_paths.size())

	# Test 8: New sounds have correct file paths
	_assert(AudioManager._sound_paths["soul_loyal"] == "res://assets/audio/soul/soul_loyal.wav", "soul_loyal path correct")
	_assert(AudioManager._sound_paths["soul_optimistic"] == "res://assets/audio/soul/soul_optimistic.wav", "soul_optimistic path correct")
	_assert(AudioManager._sound_paths["soul_passionate"] == "res://assets/audio/soul/soul_passionate.wav", "soul_passionate path correct")

	# Test 9: AudioManager can play new sounds (no crash)
	AudioManager.play_sfx("soul_loyal")
	AudioManager.play_sfx("soul_optimistic")
	AudioManager.play_sfx("soul_passionate")
	_assert(true, "New soul sounds play without crash")
	...
	# Test 11: New soul sound names are descriptive
	_assert(new_sounds[0] == "soul_lonely_whisper", "First new sound is soul_lonely_whisper")
	_assert(new_sounds[5] == "soul_nostalgic_memory", "Sixth new sound is soul_nostalgic_memory")
	_assert(new_sounds[9] == "soul_pet", "Tenth new sound is soul_pet")

	# Test 12: New soul sounds cover diverse emotions
	var emotions = ["lonely", "loyal", "magnanimous", "merciful", "motivated", "nostalgic", "optimistic", "passionate", "peaceful", "pet"]
	for emotion in emotions:
		var found = false
		for sound_name in new_sounds:
			if sound_name.find(emotion) >= 0:
...
## ============================================
## New Soul Sound Integration Tests (Round 7)
## ============================================
func _test_new_soul_sounds_round7() -> void:
	print("\n--- New Soul Sound Integration Tests (Round 7) ---")

	# Test 1: New soul sound files exist
	var new_sounds = ["soul_play", "soul_playful", "soul_proud", "soul_proud_pose", "soul_reliable", "soul_relieved_sigh", "soul_resentful", "soul_righteous", "soul_sad_hum", "soul_satisfied_purr"]
	for sound_name in new_sounds:
		var path = "res://assets/audio/soul/%s.wav" % sound_name
		_assert(FileAccess.file_exists(path), "Sound file exists: %s" % sound_name)
	...
	# Test 3: AudioManager has soul_play sound
	_assert(AudioManager._sound_paths.has("soul_play"), "AudioManager has soul_play")

	# Test 4: AudioManager has soul_proud sound
	_assert(AudioManager._sound_paths.has("soul_proud"), "AudioManager has soul_proud")

	# Test 5: AudioManager has soul_righteous sound
	_assert(AudioManager._sound_paths.has("soul_righteous"), "AudioManager has soul_righteous")
	...
	for sound_name in AudioManager._sound_paths.keys():
		if sound_name.begins_with("soul_"):
			soul_count += 1
	_assert(soul_count >= 105, "Soul sound count >= 105 (actual: %d)" % soul_count)

	# Test 7: Total sound count increased
	_assert(AudioManager._sound_paths.size() >= 325, "Total sound count >= 325 (actual: %d)" % AudioManager._sound_paths.size())

	# Test 8: New sounds have correct file paths
	_assert(AudioManager._sound_paths["soul_play"] == "res://assets/audio/soul/soul_play.wav", "soul_play path correct")
	_assert(AudioManager._sound_paths["soul_proud"] == "res://assets/audio/soul/soul_proud.wav", "soul_proud path correct")
	_assert(AudioManager._sound_paths["soul_righteous"] == "res://assets/audio/soul/soul_righteous.wav", "soul_righteous path correct")

	# Test 9: AudioManager can play new sounds (no crash)
	AudioManager.play_sfx("soul_play")
	AudioManager.play_sfx("soul_proud")
	AudioManager.play_sfx("soul_righteous")
	_assert(true, "New soul sounds play without crash")
	...
	# Test 11: New soul sound names are descriptive
	_assert(new_sounds[0] == "soul_play", "First new sound is soul_play")
	_assert(new_sounds[5] == "soul_relieved_sigh", "Sixth new sound is soul_relieved_sigh")
	_assert(new_sounds[9] == "soul_satisfied_purr", "Tenth new sound is soul_satisfied_purr")

	# Test 12: New soul sounds cover diverse emotions
	var emotions = ["play", "playful", "proud", "reliable", "relieved", "resentful", "righteous", "sad", "satisfied"]
	for emotion in emotions:
		var found = false
		for sound_name in new_sounds:
			if sound_name.find(emotion) >= 0:
...
## ============================================
## New Soul Sound Integration Tests (Round 8)
## ============================================
func _test_new_soul_sounds_round8() -> void:
	print("\n--- New Soul Sound Integration Tests (Round 8) ---")

	# Test 1: New soul sound files exist
	var new_sounds = ["soul_shy", "soul_shy_blush", "soul_skeptical", "soul_sleep", "soul_sleep_breath", "soul_smug", "soul_spawn", "soul_supportive", "soul_surprised", "soul_surprised_gasp"]
	for sound_name in new_sounds:
		var path = "res://assets/audio/soul/%s.wav" % sound_name
		_assert(FileAccess.file_exists(path), "Sound file exists: %s" % sound_name)
	...
	# Test 3: AudioManager has soul_shy sound
	_assert(AudioManager._sound_paths.has("soul_shy"), "AudioManager has soul_shy")

	# Test 4: AudioManager has soul_sleep sound
	_assert(AudioManager._sound_paths.has("soul_sleep"), "AudioManager has soul_sleep")

	# Test 5: AudioManager has soul_surprised sound
	_assert(AudioManager._sound_paths.has("soul_surprised"), "AudioManager has soul_surprised")
	...
	for sound_name in AudioManager._sound_paths.keys():
		if sound_name.begins_with("soul_"):
			soul_count += 1
	_assert(soul_count >= 115, "Soul sound count >= 115 (actual: %d)" % soul_count)

	# Test 7: Total sound count increased
	_assert(AudioManager._sound_paths.size() >= 335, "Total sound count >= 335 (actual: %d)" % AudioManager._sound_paths.size())

	# Test 8: New sounds have correct file paths
	_assert(AudioManager._sound_paths["soul_shy"] == "res://assets/audio/soul/soul_shy.wav", "soul_shy path correct")
	_assert(AudioManager._sound_paths["soul_sleep"] == "res://assets/audio/soul/soul_sleep.wav", "soul_sleep path correct")
	_assert(AudioManager._sound_paths["soul_surprised"] == "res://assets/audio/soul/soul_surprised.wav", "soul_surprised path correct")

	# Test 9: AudioManager can play new sounds (no crash)
	AudioManager.play_sfx("soul_shy")
	AudioManager.play_sfx("soul_sleep")
	AudioManager.play_sfx("soul_surprised")
	_assert(true, "New soul sounds play without crash")
	...
	# Test 11: New soul sound names are descriptive
	_assert(new_sounds[0] == "soul_shy", "First new sound is soul_shy")
	_assert(new_sounds[5] == "soul_smug", "Sixth new sound is soul_smug")
	_assert(new_sounds[9] == "soul_surprised_gasp", "Tenth new sound is soul_surprised_gasp")

	# Test 12: New soul sounds cover diverse emotions
	var emotions = ["shy", "skeptical", "sleep", "smug", "spawn", "supportive", "surprised"]
	for emotion in emotions:
		var found = false
		for sound_name in new_sounds:
			if sound_name.find(emotion) >= 0:

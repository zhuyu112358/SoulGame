...
**全功能开发期 M2** — RTS灵魂对战竞技场。M1已完成（灵魂之家+成长可视化+184自动化测试全通过）。

- M2目标：RTS实时对战竞技场
- 竞技场场景：3张地图（default/forest/crystal），地形/障碍物系统
- 对战UI：血条/能量条/技能栏/小地图/战斗日志/结果界面
- 对战逻辑：实时移动/攻击/技能/冷却/状态效果/元素克制
- AI对手：基于状态机的AI决策（move/attack/skill/heal）
- 对战结果：经验计算/成长反馈/历史记录/胜率统计
- 完整流程：选灵魂→选地图→进入竞技场→对战→结果→成长反馈
- 架构：服务端权威接口预留 + 付费系统接口预留（外观与数值分离）
